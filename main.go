	...
	cliApp.Commands = []cli.Command{
		...
		},
		{
			...
			Flags: []cli.Flag{
				...
				cli.BoolFlag{
					Name:  "disable-validation-on-install",
					Usage: "Disable the K8s API validation while running helm-diff on releases being newly installed on this apply. Useful when any release contains custom resources for CRDs that is not yet installed onto the cluster",
				},
				cli.BoolFlag{
					Name:  "include-tests",
					Usage: "enable the diffing of the helm test hooks",
...
func (c configImpl) DisableValidationOnInstall() bool {
	return c.c.Bool("disable-validation-on-install")
}
